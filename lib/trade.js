		...
		a.getOrders(authedClient).then( orders => {
			...
			/* Else if there's no open orders place one */
				...
				if(lastFill.side !== 'buy'){
					...
					console.log('low: ', lastTradePrice, 'lowest: ', lowest, 'last_trade: ', lastFillPrice, 'goal: ', (lastFillPrice - MARGIN))
				...
				if(lastFill.side !== 'sell') {
					if(highest == undefined) {
				 	...
				 	if(lastTradePrice > highest) {
				  	...
				  	console.log('high: ', lastTradePrice, 'highest: ', highest, 'last_trade: ', lastFillPrice, 'goal: ', (lastFillPrice + MARGIN))
