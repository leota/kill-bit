  		    ...
  		    a.getLastFill(authedClient).then(res => {
  		        ...
  		        if (lastFill.side !== 'buy') {
  		            ...
  		            if (lastTradePrice > lowest + DROP && lastTradePrice < lastFillPrice - MARGIN) {
  		                a.profitBuy(authedClient, currencyAccountID, lastTradePrice)
  		            }
  		            /* RETRY SELL */
  		            if (typeof(RETRY) == 'number' && lastTradePrice > lastFillPrice + RETRY && lastTradePrice < lastFillPrice + PANIC) {
  		                a.retrySell(authedClient, assetAccountID, lastTradePrice)
  		            }
  		            /* PANIC BUY */
  		            if (typeof(PANIC) == 'number' && lastTradePrice > lastFillPrice + PANIC) {
  		                a.panicBuy(authedClient, currencyAccountID, lastTradePrice)
  		            }
  		        }
  		        if (lastFill.side !== 'sell') {
  		            ...
  		            if (lastTradePrice < highest - DROP && lastTradePrice > lastFillPrice + MARGIN) {
  		                a.profitSell(authedClient, assetAccountID, lastTradePrice)
  		            }
  		            /* RETRY BUY */
  		            if (typeof(RETRY) == 'number' && lastTradePrice > lastFillPrice + RETRY && lastTradePrice > lastFillPrice - PANIC) {
  		                a.retryBuy(authedClient, currencyAccountID, lastTradePrice)
  		            }
  		            /* PANIC SELL */
  		            if (typeof(PANIC) == 'number' && lastTradePrice < lastFillPrice - PANIC) {
  		                a.panicSell(authedClient, assetAccountID, lastTradePrice)
  		            }
  		        }
  		    }).catch(err => {
