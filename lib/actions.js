...
a.calculateSpeed = function(publicClient, timeSlice, timeRange) {
	return new Promise((resolve, reject) => {
		publicClient.getProductHistoricRates({granularity: timeSlice}, function(err, response) {
		  if (err) {
		    return reject(err)
		  }
		  ...
		  for(let i = 0; i < timeRange; i++) {
		  	let speed = res[i][high] - res[i][low]
		  	speedArr.push(speed)
		  }
		  let avgSpeed = a.getAverage(speedArr).toFixed(2)
		  return resolve(avgSpeed);
		});
	})
}

a.getAverage = function(array) {
	let total = 0
