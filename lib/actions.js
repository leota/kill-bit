...
a.profitBuy = function(authedClient, currencyAccountID) {
...
a.profitSell = function(authedClient, assetAccountID) {
...
a.retryBuy = function(authedClient, currencyAccountID) {
	highest = lastTradePrice
	a.getCurrencyCapital(authedClient, currencyAccountID).then( res => {
		let available = parseFloat(res.available)
			limitPrice = lastTradePrice - 0.1
...
a.retrySell = function(authedClient, assetAccountID) {
	lowest = lastTradePrice
	a.getAssetCapital(authedClient, assetAccountID).then( res => {
		let available = parseFloat(res.available)
			limitPrice = lastTradePrice + 0.1
...
a.panicBuy = function(authedClient, currencyAccountID) {
	a.getCurrencyCapital(authedClient, currencyAccountID).then( res => {
		let available = parseFloat(res.available)
			limitPrice = lastTradePrice - 0.1
			size = ((available - 0.5) / limitPrice).toFixed(8)
...
a.panicSell = function(authedClient, assetAccountID) {
	a.getAssetCapital(authedClient, assetAccountID).then( res => {
		let available = parseFloat(res.available)
			limitPrice = lastTradePrice + 0.1
			size = (available - 0.001).toFixed(8)
